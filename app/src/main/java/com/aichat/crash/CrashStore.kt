...
class CrashStore(
    ...
    /**
     * 目录里**没能显示出来**的文件数（[list] 跳过了几个）。
     *
     * 跳过本身是对的（见 [list] 的 KDoc）：诊断页绝不能因为一份坏文件打不开。
     * 但**空列表有两种来源**，而它们该说的话不一样：
     *
     * - 一个都没跳过 ⇒ 这个目录真的是空的 ⇒「它还没崩过」**可以说**
     * - 跳过了几份 ⇒ 有东西在这儿，只是**读不出来**（名字对不上、内容读不出、
     *   大到不可能是我们写的）⇒ 这时说「还没崩过」是**替它下结论**
     *
     * 单独一个方法，而不是让 [list] 返回一对值：`list` 有十来处调用点（含测试），
     * 为一个空状态文案去改它的签名不划算。多扫一遍目录的代价是有界的
     * （最多 [maxFiles] 份 + 少量杂物）。
     */
    fun skippedCount(): Int {
        val total = dir.listFiles().orEmpty().count { it.isFile }
