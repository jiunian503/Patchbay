...
android {
    ...
    defaultConfig {
        ...
        // v1.4 之后**没有加功能**，攒的全是自查出来的修正：一类是**说出去的话不实**
        // （「不会经过任何服务器」「会说不知道」这类替平台或替模型打包票的句子），
        // 另一类是**静默失配 / 悄悄截断**的 bug（每一条都有测试守着）。
        // 所以这一版是 1.5。
        versionCode = 6
        versionName = "1.5"
    ...
    buildTypes {
        release {
            ...
            // 它们是**未压缩存储**的（`extractNativeLibs=false`，运行时直接 mmap），
            ...
            // ## 为什么 debug 不跟着砍
            //
            // 因为 instrumented test 跑在 **debug** 包上，而开发机上的模拟器
            // 是 x86_64（MuMu 12，`ro.product.cpu.abi = x86_64`）。
