...
## 动坐标之前先取一次 `wm size`，别凭印象

这个脚本打印的坐标是**设备像素**，而屏幕尺寸随设备、随方向变 —— #6 的 MuMu 12 是
**720x1280**，不是 1080x1920（后者是另一台机器上的实测值，别拿来推）。

**起点超出屏幕的 `input swipe` 不报错，是整个手势被丢掉。** 现象看起来就是
「这个页面滚不动」，很容易误判成「LazyColumn 里没内容」或「滑动参数不对」，
然后去查一个不存在的问题 —— 实测为此白查了一轮。

所以 `tap` / `tapx` 之前先跑一句：

```bash
adb -s $ADB_DEVICE shell wm size     # 720x1280 之类，随设备和方向变
```

`wm size` 报的是物理尺寸，屏幕旋转时它不变，所以还要和 `dump -b` 里 bounds 的
最大值对一下 —— 两者必须能互相解释，否则坐标就是错的。
...
5. **`input keyevent 111`（ESC）会连弹窗一起关掉**，不只是收键盘。
   在普通表单里它看起来只是「收起输入法」，于是很容易把它当成通用的
   「打完字收键盘」手段；但同一个键在 `AlertDialog` 上等于**取消** ——
   弹窗直接没了，而且**不报错**，看起来就像「填了地址点了确定但什么都没发生」。
   实测为此白走一轮（服务商表单里用得好好的收键盘动作，到「从网址安装」
   弹窗上把整个弹窗取消了）。
   要收键盘又不动弹窗，就**别按 ESC**：打完字直接 dump 定位按钮再点，
   或者用 `keyevent 66`（ENTER）—— 后者只在输入框真的声明了 IME action 时才有效。

6. **`cmd clipboard` 不存在**（这台设备上报 `No shell command implementation`）。
   所以「从剪贴板」这条导入路径**没法用 adb 直接灌内容**。要覆盖它，要么在
